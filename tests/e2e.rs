...
/// Clear any previous trace data so the test starts clean.
fn clear_collector_traces() {
    let path = Path::new(TRACES_FILE);
    if path.exists() {
        std::fs::write(path, "").expect("failed to clear traces file");
    }
}

/// Wait for the collector to be healthy by checking that the output directory
/// exists and the gRPC port is listening.
...
async fn traces_contain_enriched_resource_attributes() {
    wait_for_collector();
    clear_collector_traces();

    // Bootstrap telemetry — exports to localhost:4317 (the collector)
    let handles = otel_bootstrap::init_telemetry("e2e-test-service")
        .expect("init_telemetry failed");

    // Emit a span via the OpenTelemetry API directly to ensure it reaches the exporter
    ...
    // Explicitly shut down the tracer provider to flush the batch exporter.
    handles
        .tracer_provider
        .shutdown()
    ...
    assert!(!raw.trim().is_empty(), "collector wrote no trace data — is it running?");
    ...
    for line in raw.lines() {
        let doc: Value = serde_json::from_str(line)
            .unwrap_or_else(|e| panic!("invalid JSON from collector: {e}\nline: {line}"));
        ...
        let Some(spans) = resource_spans else { continue };
        ...
        for rs in spans {
            ...
            for attr in attrs {
                ...
                match key {
                    "service.name" => {
                        let val = attr.pointer("/value/stringValue")
                    ...
                    "host.name" => {
                        let val = attr.pointer("/value/stringValue")
                            .or_else(|| attr.pointer("/value/string_value"))
                            .and_then(|v| v.as_str());
                        assert!(val.is_some_and(|v| !v.is_empty()), "host.name should be non-empty");
                        found_host = true;
                    }
                    "process.pid" => {
                        let val = attr.pointer("/value/intValue")
